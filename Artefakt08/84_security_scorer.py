...
def calculate_security_score():
    ...
    # Finalna punktacja (nie schodzimy poniżej 0)
    final_score = max(0, score)
    status = "REJECTED (Aplikacja niebezpieczna)" if final_score < 50 else "ACCEPTED (Warunkowo)"

    # Zapis do pliku (Artefakt 8.4)
    with open("84_risk_score.txt", "w", encoding="utf-8") as f:
        f.write(f"FINAL SCORE: {final_score}/100\n")
        f.write(f"STATUS: {status}\n\n")
        f.write("LISTA POTRĄCEŃ:\n" + "\n".join(deductions))

    print(f"\n📊 WYNIK KOŃCOWY: {final_score}/100")
    print(f"[X] STATUS: {status}")
    print("\nSzczegóły zapisano w: 84_risk_score.txt")
